...
function Banana({ className, style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <motion.svg
      ...
      viewBox="0 0 48 48"
      fill="none"
      animate={{ ...floatAnimation(Math.random() * 2, 4 + Math.random() * 2, -8 - Math.random() * 6), ...rotateAnimation(Math.random() * 2, 6, 15) }}
    >
      <path
...
function PalmLeaf({ className, style, flip }: { className?: string; style?: React.CSSProperties; flip?: boolean }) {
  return (
    <motion.svg
      ...
      viewBox="0 0 80 80"
      fill="none"
      animate={floatAnimation(Math.random() * 3, 5 + Math.random() * 2, -6)}
    >
      <path
...
function MonkeyFace({ className, style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <motion.svg
      ...
      viewBox="0 0 64 64"
      fill="none"
      animate={floatAnimation(0.5, 3.5, -10)}
    >
      {/* Ears */}
      ...
      {/* Eyes */}
      <motion.g animate={{ scaleY: [1, 0.1, 1], transition: { duration: 0.3, delay: 3, repeat: Infinity, repeatDelay: 4 } }}>
      ...
      <motion.path
        ...
        fill="none"
        strokeLinecap="round"
        animate={{ d: ["M27 38 Q32 43 37 38", "M27 37 Q32 44 37 37", "M27 38 Q32 43 37 38"], transition: { duration: 2, repeat: Infinity } }}
      />
    </motion.svg>
  );
}
